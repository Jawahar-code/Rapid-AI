  ...
  return (
    <div className='h-full p-6 flex items-start flex-col lg:flex-row gap-6 text-slate-700 dark:text-slate-200 transition-colors overflow-hidden'>
      ...
      <form
        ...
        className='w-full lg:w-[38%] p-6 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm shrink-0 flex flex-col max-h-full overflow-y-auto custom-scrollbar'
      >
        {/* Header */}
        <div className='flex items-center gap-3 mb-6'>
          ...
          <div>
            ...
            <p className='text-xs text-slate-500 dark:text-slate-400'>Research Papers & General Documents • NLP + AI</p>
          </div>
        </div>

        {/* What it does */}
        <div className='mb-5 p-3.5 rounded-lg bg-sky-50 dark:bg-sky-950/20 border border-sky-100 dark:border-sky-900/40 text-xs text-sky-800 dark:text-sky-300 space-y-1'>
          <p className='font-semibold flex items-center gap-1.5'><Info className='w-3.5 h-3.5' /> What this tool does</p>
          <ul className='pl-4 list-disc space-y-0.5 text-sky-700 dark:text-sky-400'>
            <li>Auto-detects Research Paper vs. General Document</li>
            <li>Computes word count, readability, and keyword density</li>
            <li>Extracts top keywords using NLP frequency analysis</li>
            <li>Generates structured AI breakdown tailored to doc type</li>
          </ul>
        </div>
        ...
        <div className='mb-5'>
          ...
          <p className='text-[11px] text-gray-400 dark:text-slate-500 mt-1'>
            Max 10MB • Text-based PDF only (not scanned images)
          </p>
        </div>
        ...
        {docFile && (
          <div className='mb-5 flex items-center gap-2.5 p-3 bg-slate-50 dark:bg-slate-900/40 rounded-lg border border-slate-200 dark:border-slate-700'>
            ...
            <div className='flex-1 min-w-0'>
              <p className='text-xs font-medium text-slate-700 dark:text-slate-200 truncate'>{docFile.name}</p>
              <p className='text-[11px] text-slate-400'>{(docFile.size / 1024).toFixed(1)} KB</p>
            </div>
          </div>
        )}
            ...
            <div className='flex bg-slate-100 dark:bg-slate-700/60 p-0.5 rounded-xl text-xs w-full'>
              {tabs.map(({ id, label, Icon }) => (
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg font-medium transition cursor-pointer ${
                    activeTab === id
                    ...
                    <div className='h-2 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden'>
                      <div
                        className={`h-full rounded-full transition-all duration-700 ${
                          (readability.fleschReadingEase ?? 0) >= 70
                            ? 'bg-emerald-500'
